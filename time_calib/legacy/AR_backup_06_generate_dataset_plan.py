...
# from .AR_03_align_trajectory import load_frames_from_mp4


# %%
def get_bool_segments(bool_seq):
    """segment a bool list according to its value
    0001110011110000 -> 000  111  00  1111  0000"""
...
@click.option('-ar', '--use_arcap_trajectory', is_flag=True, default=False, help="use aligned_arcap_poses.csv")
@click.option('-ng', '--no_gripper_width', is_flag=True, default=False, help="do not include gripper state")
...
def main(input, output, tcp_offset,
         ...
         use_arcap_trajectory, no_gripper_width, gripper_threshold):
    # %% stage 0
    # gather inputs
    ...
    trajectory_file = "camera_trajectory.csv" if not use_arcap_trajectory else "aligned_arcap_poses.json"
    ...
    for cam_serial, gripper_ids in cam_serial_gripper_ids_map.items():
        ...
        tmp = [x for x in gripper_ids if x >= 0]
        if len(tmp) == 0:
            gripper_id = -1
        else:
            gripper_id = collections.Counter(tmp).most_common()[0][0]
    ...
    # if n_gripper_cams <= 0:
    #     # no gripper camera
    #     print(cam_serial_gripper_ids_map)
    ...
    for demo_idx, demo_data in enumerate(demo_data_list):
        ...
        for vid_idx in video_idxs:
            ...
            csv_path = vid_dir.joinpath(trajectory_file)
            if not csv_path.is_file():
                # no tracking data
                break
    ...
    for demo_idx, demo_data in enumerate(demo_data_list):
        ...
        for cam_idx, row in demo_video_meta_df.iterrows():
            ...
            if not json_path.is_file():
                print(f"Skipping {video_dir.name}, no {trajectory_file}.")
            ...
            all_is_valid.append(np.ones(n_frames, dtype=bool))
        ...
        for s, is_valid in zip(segment_slices, segment_type):
            ...
            for cam_idx, row in demo_video_meta_df.iterrows():
                if cam_idx < n_gripper_cams:
                    ...
                    width = all_gripper_widths[cam_idx][start:end]
                    if no_gripper_width:
                        width = np.ones_like(width) * 0.10
