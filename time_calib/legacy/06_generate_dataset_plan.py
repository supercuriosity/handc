...
def main(input, output, tcp_offset, tx_slam_tag,
         ...
         use_arcap_trajectory, no_gripper_width):
    # %% stage 0
    # gather inputs
    ...
    trajectory_file = "camera_trajectory.csv" if not use_arcap_trajectory else "aligned_arcap_poses.csv"
        ...
        for cam_idx, row in demo_video_meta_df.iterrows():
            ...
            for td in tag_detection_results:
                width = get_gripper_width(td['tag_dict'], 
                    ...
                    nominal_z=nominal_z, z_tolerance=None)
                if width is not None:
                    gripper_timestamps.append(td['time'])
                    gripper_widths.append(gripper_cal_interp(width))
            
            
            gripper_widths = medfilt(gripper_widths, kernel_size=7)  # added by xuyue
