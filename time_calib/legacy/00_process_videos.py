...
@click.option('--ignore_gripper_calib', is_flag=True, default=False, help="")
def main(session_dir, ignore_gripper_calib):
    for session in session_dir:
        ...
        if not input_dir.is_dir():
            input_dir.mkdir()
            raise FileNotFoundError(f"{input_dir.name} subdir don't exits! Creating one and moving all mp4 videos inside.")
        ...
        if (not mapping_vid_path.exists()) and not(mapping_vid_path.is_symlink()):
            raise FileNotFoundError(f"raw_videos/mapping.mp4 don't exist!")
            max_size = -1
            max_path = None
        ...
        if not ignore_gripper_calib and not gripper_cal_dir.is_dir():
            gripper_cal_dir.mkdir()
            raise FileNotFoundError("raw_videos/gripper_calibration don't exist! Creating one with the first video of each camera serial.")
