...
class LeftImagePublisher(Node):
    def __init__(self):
        super().__init__('camera_node')
        self.publisher_ = self.create_publisher(Image, '/camera/tactile_left', 10)
        timer_period = 0.05  # 20 FPS
        self.timer = self.create_timer(timer_period, self.timer_callback)

        self.cap = cv2.VideoCapture("/dev/video0") # 0 代表第一个摄像头
        if not self.cap.isOpened():
            self.get_logger().error('Could not open video capture device at index 0')
            rclpy.shutdown()

        self.bridge = CvBridge()
        self.get_logger().info('Left Tactile Camera node has been started and is publishing.')
...
def main(args=None):
    ...
    image_publisher = LeftImagePublisher()
    try:
