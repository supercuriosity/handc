...
def get_gripper_width(tag_dict, left_id, right_id, nominal_z=0.075, z_tolerance=None):
    assert z_tolerance is None, "for non-parallel gripper"
    ...
    if left_id in tag_dict:
        ...
        if tvec.shape == () and tvec == None:
            return None
        if z_tolerance is None or zmin < tvec[-1] < zmax:
            left_x = tvec[0]
    ...
    if (left_x is not None) and (right_x is not None):
        ...
        if width < 0:
            print("Warning: gripper width %.5f, you may need to swap left and right arucos"%width)
            # width = -width
            return None
...
def draw_predefined_mask(img, color=(0,0,0), mirror=True, gripper=True, finger=True, use_aa=False):
    raise NotImplementedError("draw_predefined_mask")
    all_coords = list()
    if mirror:
...
def get_gripper_with_finger_mask(img, height=0.37, top_width=0.25, bottom_width=1.4, color=(0,0,0)):
    raise NotImplementedError("get_gripper_with_finger_mask")
    # image size
    img_h = img.shape[0]
    img_w = img.shape[1]
