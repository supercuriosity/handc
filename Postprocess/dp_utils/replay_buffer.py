...
class ReplayBuffer:
    ...
    def __init__(self, 
        ...
        if 'n_variants_per_original' in root['meta']:
            for key, value in root['data'].items():
                # print(root['meta']['episode_ends'][-1])
                assert(value.shape[0] == root['meta']['episode_ends'][-1] or value.shape[0] == root['meta']['episode_ends'][-1]* root['meta']['n_variants_per_original']), f"Shape mismatch {value.shape[0]} != {root['meta']['episode_ends'][-1]}"
        else:   
            for key, value in root['data'].items():
                assert(value.shape[0] == root['meta']['episode_ends'][-1]), f"Shape mismatch {value.shape[0]} != {root['meta']['episode_ends'][-1]}"
        self.root = root
